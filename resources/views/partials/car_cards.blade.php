                        ...
                        <div class="wrap-hover-listing">
                            ...
                            @foreach($images as $index => $img)
                                <div class="listing-item {{ $index == 0 ? 'active' : '' }}" title="{{ $car->title }}">
                                    <div class="images">
                                        <img src="{{ asset('storage/' . $img->path) }}" class="swiper-image" alt="{{ $car->title }}">
                                    </div>
                                </div>
                            @endforeach
                            @if($car->images->count() > 3)
                                <div class="listing-item view-gallery">
                                    <div class="images">
                                        ...
                                        <img src="{{ asset('storage/' . $car->images->first()->path) }}" alt="more">
...
@else
    <div class="col-12 text-center py-5">
        <p>Автомобилей не найдено. Измените параметры фильтра.</p>
    </div>
@endif
